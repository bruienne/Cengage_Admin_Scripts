...
parser = OptionParser(
    'Usage: %prog [/path/to/manifest] [-o | --optional-software] [-n | --nested-manifests] \
    [-u | --upload http://mymunkiserver/{UNIT}/packages/batch ]')
    ...
    if os.path.isfile(manifestpath):
        ...
        if processnested:
            ...
            for thismanifest in nestedmanifests:
                managedinstalls = managedinstalls + processManifest(os.path.join(manifestdir, thismanifest),
                                                                    nested=True)
...
def main():
    """
    We call processManifest and store the results in itemstobundle which is then passed to
    bundleItems to do the final processing.
    """
